...
function ButtonArrow({ onClick, title, outlined }) {
    return (
        <>
            {/* <!-- Base - Right --> */}
            {outlined === 'false' ? (
                <button
                    type="button"
                    className={`z-0 relative inline-flex items-center px-8 py-3 overflow-hidden text-white bg-${BlueMain} rounded-full  group focus:bg-${BlueSec} active:bg-${BlueMain} focus:outline-none focus:ring`}
                    onClick={() => onClick()}
                >
